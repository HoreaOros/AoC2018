...
//                //List<List<(int r, int c)>> minPaths = unit.GetAllMinPaths(chosenTarget, map, LIN, COL);
...
//                        if (map[nr, nc] == '.' && IsPath((chosenTarget.r, chosenTarget.c), (nr, nc)) == minDistance - 1)
//                            steps.Add((nr, nc));
//                }
...
//int IsPath((int r, int c) start, (int r, int c) end)
//{
//    if (start == end)
//        return 0;
...
//Console.WriteLine(rounds);
//Console.WriteLine(units.Where(x => x.IsAlive).Sum(x => x.HitPoints));
//Console.WriteLine((rounds) * units.Where(x => x.IsAlive).Sum(x => x.HitPoints));
